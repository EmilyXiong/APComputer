...
public class Hashing {
	public static void main(String[] args) {
		int arrayLength = Integer
				.parseInt(JOptionPane.showInputDialog("Hashing!\n" + "Enter the size of the array:  "));// 20

		int numItems = Integer.parseInt(JOptionPane.showInputDialog("Add n items:  ")); // 15

		int scheme = Integer
				.parseInt(JOptionPane.showInputDialog("The Load Factor is " + (double) numItems / arrayLength
...
class HashtableRehash implements Hashtable {
	...
	public HashtableRehash(int size) {
		...
		array = new Object[size];
		constant = findRelativelyPrime(size);
	...
	public int findRelativelyPrime(int n) {
		int retValue = 2;
		while (retValue < n) {
			if (isPrime(retValue) && (n%retValue) != 0) {
				break;
			}
			retValue++;
		}
